...
class NYCSViewController: UIViewController {
        ...
        let task = session.dataTask(with: request) { [weak self] (highSchoolsData, response, error)  in
            if highSchoolsData != nil {
                do {
                    ...
                    self?.nycHSList = Utilities.fetchNYCHsWithJsonData(highSchoolsObject)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Constants.HSWithSATScoreSegue{
            let highSchoolWithSATScoreVC = segue.destination as! NYCSDetailViewController
...
extension NYCSViewController: UISearchResultsUpdating {
...
extension NYCSViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    //MARK: - UITable Cell for Row
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: NYCSTableViewCell = self.tableView.dequeueReusableCell(withIdentifier: Constants.hsCellIdentifier, for: indexPath) as! NYCSTableViewCell
